...
import com.tamseo.ws.AbstractControllerTest;
 ...
 * 
 * These tests utilize the Spring MVC Mock objects to simulate sending actual HTTP requests to the
 * Controller component. This test ensures that the RequestMappings are configured correctly. Also,
 * these tests ensure that the request and response bodies are serialized as expected.
 * 
 * @author Matt Warman
 */
@Transactional
public class GreetingControllerTest extends AbstractControllerTest {
  ...
  public void testGetGreeting() throws Exception {
    ...
    MvcResult result =
        mvc.perform(MockMvcRequestBuilders.get(uri, id).accept(MediaType.APPLICATION_JSON))
            .andReturn();
  ...
  public void testUpdateGreeting() throws Exception {
    ...
    MvcResult result =
        mvc.perform(
            MockMvcRequestBuilders.put(uri, id).contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON).content(inputJson)).andReturn();
    ...
    Assert.assertEquals("failure - expected greeting.id unchanged", greeting.getId(),
        updatedGreeting.getId());
    Assert.assertEquals("failure - expected updated greeting text match", updatedText,
